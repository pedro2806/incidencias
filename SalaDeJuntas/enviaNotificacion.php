...
header('Content-Type: text/html; charset=UTF-8');
    ...
    $sql_correo = "SELECT r.id, r.id_usuario, r.fecha_hora_fin, u.correo, r.descripcion, u.nombre
                   FROM reservas r
                   INNER JOIN usuarios u ON r.id_usuario = u.noEmpleado
                   WHERE r.estatus = 'Reservada'
                   AND TIMESTAMPDIFF(MINUTE, NOW(), r.fecha_hora_fin) <=10 
                   AND TIMESTAMPDIFF(MINUTE, NOW(), r.fecha_hora_fin)>=0";
                   echo $sql_correo;
    $resul_correo = $conn->query($sql_correo);
    
    while ($row2 = $resul_correo->fetch_assoc()) {
        echo $usuario;
        $usuario = $row2["correo"];
    ...
    $deAsunto="Tiempo Sala de Juntas.";
    ...
    $mail->SetFrom("mess.metrologia@gmail.com", "Tiempo Sala de Juntas.");
...
<body>
    ...
    <center>
    <h1>
        Te recordamos que tu tiempo en la Sala de Juntas esta por terminar. Por favor, aseg&uacute;rate de salir a tiempo.;
        <br>
    </h1>
    ...
    if (isset($usuario)){ 
        ...
        echo json_encode(["status" => "error", "message" => "No se encontraron correos."]);
        exit;
    }
